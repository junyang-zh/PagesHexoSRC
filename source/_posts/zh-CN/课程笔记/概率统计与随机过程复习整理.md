...
随机现象的观察（**随机试验**） $E$ 的**样本空间**为 $\Omega=\{\omega\}$，**随机事件** $A\sub \Omega$。**必然事件** $\Omega$，**不可能事件** $\varnothing$
...
- 古典概型 $P(A)=\frac{text{ sum of}Atext{ results}}{text{ total results}}$
...
可减性 $A\sub B \Rightarrow P(B-A)=P(B)-P(A)$

取反性 $P\left(\overline A\right)=1-P(A)$

容斥原理 $P(\bigcup_i A_i)=\sum_{S\sub\{A_i\}}(-1)^{|S|-1}P(\bigcap_{s\in S}s)$
...
$A,B text{ are independent} \Longleftrightarrow A,\overline B text{ are independent} \Longleftrightarrow \overline A,B text{ are independent} \Longleftrightarrow \overline A,\overline B text{ are independent}$
...
随机变量独立 $F(x,y)=F_X(x)F_Y(y) \Longleftrightarrow f(x,y)=f_X(x)f_Y(y)text{ holds almost everywhere} \Longleftrightarrow X,Ytext{ are independent} $
...
\xlongequal{X text{  and } Y text{  are independent}}
...
I 类错误概率 $\alpha(\mu)=P\{text{ rejects}H_0|H_0text{ is true}\}$

II 类错误概率 $\beta(\mu)=P\{text{ accepts}H_0|H_0text{ is false}\}$
